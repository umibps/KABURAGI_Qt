...
/* Utils */
static INLINE FLOAT_T sqlen(GRAPHICS_POINT_FLOAT p0, GRAPHICS_POINT_FLOAT p1)
{
	GRAPHICS_POINT_FLOAT delta;
...
*/
static INLINE int sqsteps2shift(FLOAT_T steps_sq)
{
	int r;
	frexp(MAXIMUM(1.0, steps_sq), &r);
	return (r + 1) >> 1;
}
...
*/
static INLINE void fd_init(FLOAT_T x, FLOAT_T y, FLOAT_T z, FLOAT_T w, FLOAT_T f[4])
{
	f[0] = x;
	f[1] = w - x;
	f[2] = 6. * (w - 2. * z + y);
...
* the output f.
*/
static INLINE void fd_down(FLOAT_T f[4])
{
	f[3] *= 0.125;
	f[2] = f[2] * 0.25 - f[3];
	f[1] = (f[1] - f[2]) * 0.5;
...
*/
static INLINE void fd_fwd(FLOAT_T f[4])
{
	f[0] += f[1];
	f[1] += f[2];
	f[2] += f[3];
...
*/
static INLINE void fd_fixed(FLOAT_T d[4], int32 i[4])
{
	i[0] = GraphicsFixed16_16FromFloat(256 *  2 * d[0]);
	i[1] = GraphicsFixed16_16FromFloat(256 * 16 * d[1]);
	i[2] = GraphicsFixed16_16FromFloat(256 * 16 * d[2]);
	i[3] = GraphicsFixed16_16FromFloat(256 * 16 * d[3]);
}
...
*/
static INLINE FLOAT_T bezier_steps_sq(GRAPHICS_POINT_FLOAT p[4])
{
	FLOAT_T tmp = sqlen (p[0], p[1]);
	tmp = MAXIMUM(tmp, sqlen (p[2], p[3]));
	tmp = MAXIMUM(tmp, sqlen (p[0], p[2]) * .25);
	tmp = MAXIMUM(tmp, sqlen (p[1], p[3]) * .25);
	return 18.0 * tmp;
}
...
*/
static INLINE void split_bezier_1D(
	FLOAT_T  x, FLOAT_T  y, FLOAT_T  z, FLOAT_T  w,
	FLOAT_T *x0, FLOAT_T *y0, FLOAT_T *z0, FLOAT_T *w0,
	FLOAT_T *x1, FLOAT_T *y1, FLOAT_T *z1, FLOAT_T *w1
)
{
	FLOAT_T tmp;
...
* nodes.
*/
static void split_bezier(
	GRAPHICS_POINT_FLOAT p[4],
	GRAPHICS_POINT_FLOAT fst_half[4],
	GRAPHICS_POINT_FLOAT snd_half[4]
)
{
...
*/
static INLINE int intersect_interval(FLOAT_T a, FLOAT_T b, FLOAT_T c, FLOAT_T d)
{
	if(c <= a && b <= d)
	{
		return INSIDE;
...
)
{
	if(LIKELY(0 <= x && 0 <= y && x < width && y < height))
	{
		...
		*((uint32*) (data + y*(ptrdiff_t)stride + 4*x)) = ((ta << 16) & 0xff000000) |
...
*/
static INLINE void rasterize_bezier_curve(
...
)
{
	...
	*/
	x0 = GraphicsFixedFromFloat(dxu[0]);
	y0 = GraphicsFixedFromFloat(dyu[0]);
	xu[0] = 0;
	yu[0] = 0;
	...
	for (u = 0; u <= usteps; ++u)
	{
		...
		*/
		...
		int x = GraphicsFixedIntegerFloor(x0 + (xu[0] >> 15) + ((xu[0] >> 14) & 1));
		int y = GraphicsFixedIntegerFloor(y0 + (yu[0] >> 15) + ((yu[0] >> 14) & 1));
...
{
	FLOAT_T top, bottom, left, right, steps_sq;
	int i, v;

	top = bottom = p[0].y;
	for(i = 1; i < 4; ++i)
	{
		top	= MINIMUM(top,	p[i].y);
		bottom = MAXIMUM(bottom, p[i].y);
	}
	...
	for(i = 1; i < 4; ++i)
	{
		left  = MINIMUM(left,  p[i].x);
		right = MAXIMUM(right, p[i].x);
	}
	...
	if(steps_sq >= (v == INSIDE ? STEPS_MAX_U * STEPS_MAX_U : STEPS_CLIP_U * STEPS_CLIP_U))
	{
		...
		*/
		GRAPHICS_POINT_FLOAT first[4], second[4];
		FLOAT_T midc[4];
	...
	else
	{
		...
		rasterize_bezier_curve(data, width, height, stride, ushift,
			xu, yu,
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c0[0]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c0[1]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c0[2]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c0[3]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[0]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[1]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[2]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[3]));

		/* Draw the end point, to make sure that we didn't leave it
		* out because of rounding */
		draw_pixel (data, width, height, stride,
			GraphicsFixedIntegerFloor(GraphicsFixedFromFloat(p[3].x)),
			GraphicsFixedIntegerFloor(GraphicsFixedFromFloat(p[3].y)),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[0]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[1]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[2]),
			GRAPHICS_COLOR_FLOAT_TO_SHORT(c3[3]));
...
* [0,1] (including both extremes).
*/
static INLINE void rasterize_bezier_patch(
	unsigned char* data,
	int width,
	int height,
	int stride,
	int vshift,
	GRAPHICS_POINT_FLOAT p[4][4],
	FLOAT_T col[4][4]
)
{
	FLOAT_T pv[4][2][4], cstart[4], cend[4], dcstart[4], dcend[4];
	int v, i, k;
...
draw_bezier_patch (unsigned char *data, int width, int height, int stride,
	GRAPHICS_POINT_FLOAT p[4][4], double c[4][4])
{
	...
	for (i = 0; i < 4; ++i) {
		for (j= 0; j < 4; ++j) {
			top	= MINIMUM(top,	p[i][j].y);
			bottom = MAXIMUM(bottom, p[i][j].y);
		}
	}
	...
	for (i = 0; i < 4; ++i)
	{
		for(j= 0; j < 4; ++j)
		{
			left  = MINIMUM(left,  p[i][j].x);
			right = MAXIMUM(right, p[i][j].x);
		}
	}
	...
	steps_sq = 0;
	for(i = 0; i < 4; ++i)
		steps_sq = MAXIMUM(steps_sq, bezier_steps_sq (p[i]));
	...
	if(steps_sq >= (v == INSIDE ? STEPS_MAX_V * STEPS_MAX_V : STEPS_CLIP_V * STEPS_CLIP_V))
	{
		...
		* overlapping parts with higher v. */
		...
		GRAPHICS_POINT_FLOAT first[4][4], second[4][4];
		FLOAT_T subc[4][4];
...
*/
void GraphicsMeshPatternRasterize(
...
{
	GRAPHICS_POINT_FLOAT nodes[4][4];
	FLOAT_T colors[4][4];
	GRAPHICS_MATRIX p2u;
	...
	for(i = 0; i < n; i++)
	{
		for(j = 0; j < 4; j++)
		{
			for(k = 0; k < 4; k++)
			{
				...
				GraphicsMatrixTransformPoint(&p2u, &nodes[j][k].x, &nodes[j][k].y);
