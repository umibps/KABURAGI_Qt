...
typedef struct _SELECT_TARGET_LABELS
{
	char *target, *area;
	char *rgb, *rgba, *alpha;
	char *active_layer, *under_layer, *canvas;
	char *threshold;
	char *detect_area, *area_normal, *area_large;
} SELECT_TARGET_LABELS;

typedef struct _CONTROL_POINT_LABELS
{
	char *select, *move, *change_pressure, *delete_point, *move_stroke,
		*copy_stroke, *joint_stroke;
} CONTROL_POINT_LABELS;

typedef struct _SHAPE_LABELS
{
	char *circle, *eclipse, *triangle, *square, *rhombus, *hexagon,
		*star, *pattern, *image;
} SHAPE_LABELS;
...
typedef struct _APPLICATION_LABELS
{
	...
	struct
	{
		char *title, *initialize, *new_brush, *smooth, *smooth_quality,
			...
			*use_old_anti_alias;
		char *brush_default_names[NUM_BRUSH_TYPE];
	...
	struct
	{
		...
		char *blend_mode, *opacity, *mask_with_under, *lock_opacity;
		char *blend_modes[LAYER_BLEND_SLELECTABLE_NUM];
	} layer_window;
