...
static void
PixelManipulateCPU_ID (uint32 feature,
...
detect_cpu_features (void)
{
	...
	/* Get feature bits */
	PixelManipulateCPU_ID (0x01, &a, &b, &c, &d);
	if (d & (1 << 15))
	features |= X86_CMOV;
	if (d & (1 << 23))
	features |= X86_MMX;
	...
	PixelManipulateCPU_ID (0x00, &a, &b, &c, &d);
	(void)memcpy (vendor + 0, &b, 4);
	(void)memcpy (vendor + 4, &d, 4);
	(void)memcpy (vendor + 8, &c, 4);

	if (strcmp (vendor, "AuthenticAMD") == 0 ||
		strcmp (vendor, "HygonGenuine") == 0 ||
		strcmp (vendor, "Geode by NSC") == 0)
	{
		PixelManipulateCPU_ID (0x80000000, &a, &b, &c, &d);
		if (a >= 0x80000001)
		{
		PixelManipulateCPU_ID (0x80000001, &a, &b, &c, &d);
...
#if defined(USE_X86_MMX) && USE_X86_MMX
	if (have_feature (MMX_BITS))
	imp = PixelManipulateImplementationCreateMMX(imp);
#endif

#ifdef USE_SSE2
	if (have_feature (SSE2_BITS))
	imp = PixelManipulateImplementationCreateSSE2(imp);
#endif

#ifdef USE_SSSE3
	if (have_feature (SSSE3_BITS))
	imp = PixelManipulateImplementationCreateSSSE3(imp);
#endif
