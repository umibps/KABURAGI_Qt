...
TRANSFORM_DATA *CreateTransformData(DRAW_WINDOW* window)
{
	...
	GRAPHICS_IMAGE_SURFACE before_surface;
	GRAPHICS_SURFACE_PATTERN before_pattern;
	...
	InitializeGraphicsMatrixTranslate(&matrix, - ret->x, - ret->y);
	for(i=0; i<ret->num_layers; i++)
	{
		...
		{
			GRAPHICS_SURFACE_PATTERN local_pattern;
			...
			GraphicsSetSourceSurface(&window->temp_layer->context.base, &ret->layers[i]->surface.base, 0, 0, &local_pattern);
			GraphicsMaskSurface(&window->temp_layer->context.base, &window->selection->surface.base, 0, 0);
			for(j=0; j<ret->height; j++)
			{
				(void)memcpy(
					&ret->source_pixels[i][j*copy_stride],
			...
			InitializeGraphicsImageSurfaceForData(&before_surface, ret->source_pixels, GRAPHICS_FORMAT_ARGB32,
									ret->width, ret->height, ret->stride, &window->app->graphics);
			InitializeGraphicsPatternForSurface(&before_pattern, &before_surface.base);
			GraphicsPatternSetMatrix(&before_pattern.base, &matrix);

			GraphicsSetSource(&ret->layers[i]->context.base, &before_pattern);
			GraphicsPaint(&ret->layers[i]->context.base);

			DestroyGraphicsPattern(&before_pattern.base);
			DestroyGraphicsSurface(&before_surface.base);
		}
	}
...
/*
* TransformDrawLineSelectionArea関数
* 引数
* point
* transform
* tranput
...
static void TransformOutput(TRANSFORM_DATA* transform, int current_layer)
{
	...
	for(i=0; i<4; i++)
	{
		GRAPHICS_SURFACE_PATTERN local_pattern = {0};
		...
		GraphicsMoveTo(&window->mask_temp->context.base, dst_points[0][0], dst_points[0][1]);
		GraphicsLineTo(&window->mask_temp->context.base, dst_points[1][0], dst_points[1][1]);
		GraphicsLineTo(&window->mask_temp->context.base, dst_points[2][0], dst_points[2][1]);
		GraphicsLineTo(&window->mask_temp->context.base, dst_points[3][0], dst_points[3][1]);
		GraphicsClosePath(&window->mask_temp->context.base);
		GraphicsClip(&window->mask_temp->context.base);

		GraphicsSetSourceSurface(&window->mask_temp->context.base, &window->temp_layer->surface.base, 0, 0, &local_pattern);
...
void Transform(TRANSFORM_DATA* transform)
{
	...
	InitializeGraphicsImageSurfaceForData(&surface, window->brush_buffer, GRAPHICS_FORMAT_A8, 
				window->width, window->height, window->width, &window->app->graphics);
	InitializeGraphicsDefaultContext(&context, &surface, &window->app->graphics);
	GraphicsMoveTo(&context.base, transform->trans[0][0], transform->trans[0][1]);
	for(i=1; i<8; i++)
	{
		GraphicsLineTo(&context.base, transform->trans[i][0], transform->trans[i][1]);
	}
	...
	for(i=0; i<transform->num_layers; i++)
	{
		...
		GraphicsSetOperator(&transform->layers[i]->context.base, GRAPHICS_OPERATOR_OVER);
		GraphicsSetSourceSurface(&transform->layers[i]->context.base, &window->mask_temp->surface.base, 0, 0, &local_pattern);
		GraphicsMaskSurface(&transform->layers[i]->context.base, &surface.base, 0, 0);
	}
...
void ProjectionTransform(TRANSFORM_DATA* transform, FLOAT_T projection_param[4][9])
{
	...
	for(i=0; i<transform->num_layers; i++)
	{
		...
		GraphicsSetOperator(&transform->layers[i]->context.base, GRAPHICS_OPERATOR_OVER);
		GraphicsSetSourceSurface(&transform->layers[i]->context.base,
				&window->temp_layer->surface.base, 0, 0, &local_pattern);
		GraphicsMaskSurface(&transform->layers[i]->context.base, &surface.base, 0, 0);
	}
...
void TransformButtonPress(TRANSFORM_DATA* transform, FLOAT_T x, FLOAT_T y)
{
...
void ProjectionTransformMotionNotify(
	...
	FLOAT_T x,
	FLOAT_T y,
	unsigned int mouse_key_flag
)
{
	if((mouse_key_flag & MOUSE_KEY_FLAG_LEFT) != 0)
	{
		FLOAT_T ave_x = 0, ave_y = 0;

		if((transform->last_x - x) * (transform->last_x - x) + (transform->last_y - y) * (transform->last_y - y)
			< UPDATE_DISTANCE * UPDATE_DISTANCE)
		{
			return;
		}
		transform->last_x = x,	transform->last_y = y;
		...
		case TRANSFORM_SCALE:
			{
				FLOAT_T diff_x = x - transform->before_x, diff_y = y - transform->before_y;
...
)
{
	...
	{
		ProjectionTransformMotionNotify(window, transform, x, y, mouse_key_flag);
	}
	else
	{
		TransformMotionNotify(window, transform, x, y, mouse_key_flag);
	}
}
...
{
#define DISPLAY_LINE_WIDTH 2.5
	FLOAT_T ave_x = 0, ave_y = 0;
	FLOAT_T zoom = window->zoom * 0.01;
	eGRAPHICS_LINE_CAP before_line_type;
	int i;
	...
	{
		GraphicsSetSourceRGB(&window->disp_temp->context.base, 1, 1, 1);
		GraphicsArc(&window->disp_temp->context.base,
	...
	GraphicsArc(&window->disp_temp->context.base, ave_x * zoom, ave_y * zoom,
		12 * zoom, 0, 2*M_PI);
	GraphicsStroke(&window->disp_temp->context.base);
	before_line_type = GraphicsGetLineCap(&window->disp_temp->context.base);
	GraphicsSetLineCap(&window->disp_temp->context.base, GRAPHICS_LINE_CAP_ROUND);
	GraphicsSetLineWidth(&window->disp_temp->context.base, DISPLAY_LINE_WIDTH);
	GraphicsMoveTo(&window->disp_temp->context.base, (ave_x - 9) * zoom, ave_y * zoom);
	GraphicsLineTo(&window->disp_temp->context.base, (ave_x + 9) * zoom, ave_y * zoom);
	GraphicsStroke(&window->disp_temp->context.base);
	GraphicsMoveTo(&window->disp_temp->context.base, (ave_x + 9) * zoom, ave_y * zoom);
	GraphicsLineTo(&window->disp_temp->context.base, (ave_x + 9 - 3.5) * zoom, (ave_y - 3.5) * zoom);
	GraphicsStroke(&window->disp_temp->context.base);
	GraphicsMoveTo(&window->disp_temp->context.base, (ave_x + 9) * zoom, ave_y * zoom);
	GraphicsLineTo(&window->disp_temp->context.base, (ave_x + 9 - 3.5) * zoom, (ave_y + 3.5) * zoom);
	GraphicsStroke(&window->disp_temp->context.base);
	...
	GraphicsMoveTo(&window->disp_temp->context.base, ave_x * zoom, (ave_y - 9) * zoom);
	GraphicsLineTo(&window->disp_temp->context.base, ave_x * zoom, (ave_y + 9) * zoom);
	GraphicsStroke(&window->disp_temp->context.base);
	GraphicsMoveTo(&window->disp_temp->context.base, ave_x * zoom, (ave_y - 9) * zoom);
	GraphicsLineTo(&window->disp_temp->context.base, (ave_x - 3.5) * zoom, (ave_y - 9 + 3.5) * zoom);
	GraphicsStroke(&window->disp_temp->context.base);
	GraphicsMoveTo(&window->disp_temp->context.base, ave_x * zoom, (ave_y - 9) * zoom);
	GraphicsLineTo(&window->disp_temp->context.base, (ave_x + 3.5) * zoom, (ave_y - 9 + 3.5) * zoom);
	GraphicsStroke(&window->disp_temp->context.base);
	...
	GraphicsSetLineCap(&window->disp_temp->context.base, before_line_type);
