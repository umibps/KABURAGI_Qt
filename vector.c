...
)
{
	...
	graphics = &work->window->app->graphics;
	...
	(void)InitializeGraphicsImageSurfaceForData(&ret->surface, ret->pixels, GRAPHICS_FORMAT_ARGB32,
										  width, height, ret->stride, graphics);
	(void)InitializeGraphicsDefaultContext(&ret->context, &ret->surface, graphics);
...
void DeleteVectorLineLayer(VECTOR_LINE_LAYER** layer)
{
	...
	DestroyGraphicsSurface(&(*layer)->surface.base);
	DestroyGraphicsContext(&(*layer)->context.base);
...
)
{
	...
	GRAPHICS_RADIAL_PATTERN brush;
	GRAPHICS *graphics;
	int i, x, y;

	graphics = &window->app->graphics;

	GraphicsSetOperator(&window->temp_layer->context.base, GRAPHICS_OPERATOR_OVER);
	...
	for(i=0; i<line->num_points-1; i++)
	{
		...
		do
		{
			...
			InitializeGraphicsPatternRadial(&brush, draw_x, draw_y, r * 0.0, draw_x, draw_y, r, graphics);
			GraphicsPatternSetExtend(&brush.base.base, GRAPHICS_EXTEND_NONE);
			GraphicsPatternAddColorStopRGBA(&brush.base.base, 0.0, red, green, blue, alpha);
			GraphicsPatternAddColorStopRGBA(&brush.base.base, 1.0-line->blur*0.01,
												red, green, blue, alpha);
			GraphicsPatternAddColorStopRGBA(&brush.base.base, 1.0, red, green, blue,
												line->outline_hardness*0.01*alpha);
			GraphicsSetSource(&window->temp_layer->context.base, &brush.base);

			GraphicsArc(&window->temp_layer->context.base, draw_x, draw_y, r, 0, 2*M_PI);
			...
			DestroyGraphicsPattern(&brush.base.base);
...
)
{
	...
	for(i=0; i<line->num_points-1; i++)
	{
		...
		do
		{
			...
			GraphicsSetSource(&window->temp_layer->context.base, &brush.base.base);
...
)
{
	...
	InitializeGraphicsDefaultContext(&context, &window->work_layer->surface, &window->app->graphics);
	(void)memset(window->work_layer->pixels, 0, window->pixel_buf_size);

	GraphicsTranslate(&context.base, square->left + square->width * 0.5, square->top + square->height * 0.5);
	GraphicsRotate(&context.base, square->rotate);
	GraphicsTranslate(&context.base, - square->width * 0.5, - square->height * 0.5);
	GraphicsSetLineWidth(&context.base,  square->line_width);
	GraphicsSetLineJoin(&context.base, (eGRAPHICS_LINE_JOIN)square->line_joint);
	GraphicsRectangle(&context.base, 0,
						0, square->width, square->height);
	GraphicsPathExtents(&context.base, &rectangle->min_x, &rectangle->min_y,
	...
	{
		GraphicsSetSourceRGBA(&context.base,
		...
		GraphicsStrokePreserve(&context.base);
	}
...
)
{
	...
	FLOAT_T sin_value = sin(- rhombus->rotate);
	FLOAT_T cos_value = cos(- rhombus->rotate);
	...
	GraphicsSetLineWidth(&context.base, rhombus->line_width);
	GraphicsSetLineJoin(&context.base, (eGRAPHICS_LINE_JOIN)rhombus->line_joint);
	GraphicsMoveTo(&context.base, rhombus->x - sin_value * height, rhombus->y - cos_value * height);
	GraphicsLineTo(&context.base, rhombus->x - cos_value * width, rhombus->y + sin_value * width);
	GraphicsLineTo(&context.base, rhombus->x + sin_value * height, rhombus->y + cos_value * height);
	GraphicsLineTo(&context.base, rhombus->x + cos_value * width, rhombus->y - sin_value * width);
...
)
{
	...
	(void)memset(window->work_layer->pixels, 0, window->pixel_buf_size);
	GraphicsTranslate(&context.base, eclipse->x + eclipse->radius, eclipse->y + eclipse->radius);
	GraphicsRotate(&context.base, eclipse->rotate);
	GraphicsTranslate(&context.base, - eclipse->radius, - eclipse->radius);

	GraphicsSave(&context.base);
	GraphicsScale(&context.base, x_scale, y_scale);
	GraphicsArc(&context.base, (1 - cos_value - sin_value) * eclipse->radius / x_scale,
		(1 - cos_value + sin_value) * eclipse->radius / y_scale, eclipse->radius, 0, 2 * M_PI);
	GraphicsClosePath(&context.base);
	GraphicsPathExtents(&context.base, &rectangle->min_x, &rectangle->min_y,
		&rectangle->max_x, &rectangle->max_y);
	...
	GraphicsRestore(&context.base);
	GraphicsSetLineWidth(&context.base, eclipse->line_width);
	GraphicsSetLineJoin(&context.base, (eGRAPHICS_LINE_JOIN)eclipse->line_joint);
...
)
{
	...
	if(luaL_loadstring(state, script->script_data) != 0)
	{
		...
		dialog = MessageDialogNew(message, GetMainWindowWidget(app), DIALOG_TYPE_ERROR);
		MessageDialogMoveToParentCenter(dialog);
		MessageDialogExecute(dialog);
		MessageDialogDestroy(dialog);
...
{
	uint32 length = (uint32)strlen(script->script_data);
...
void BlendVectorLineLayer(VECTOR_LINE_LAYER* src, VECTOR_LINE_LAYER* dst)
{
	...
	GraphicsPaint(&dst->context.base);
	...
	else if((layer->flags & VECTOR_LAYER_RASTERIZE_ACTIVE) != 0)
	{
		...
		while(src != NULL)
		{
			if((void*)src == layer->active_data)
			{
				...
				{
					GRAPHICS_SURFACE_PATTERN pattern = {0};
					GraphicsSetSourceSurface(&dst->base_data.layer->context.base,
												&window->work_layer->surface.base, 0, 0, &pattern);
					GraphicsPaint(&dst->base_data.layer->context.base);
					DestroyGraphicsPattern(&pattern.base);
				}
			}
			else
...
{
	// ベクトルレイヤーのデータ
	VECTOR_LAYER *layer = target->layer_data.vector_layer;
	// ベクトルレイヤーの基本情報読み込み用
...
)
{
	...
	uint32 num_line = 0;
	// ラインデータ書き出し用
	VECTOR_LINE *line = (VECTOR_LINE*)((VECTOR_BASE_DATA*)target->layer_data.vector_layer->base)->next;
	// 4バイト書き出し用
	uint32 data32;
	// for文用のカウンタ
	unsigned int i;
	...
	// ベクトルの座標、サイズ、筆圧、色を一度メモリに溜める
		// ラインの数を書き込む領域を開けておく
	(void)MemSeek(data_stream, sizeof(uint32), SEEK_SET);
	// ベクトルレイヤーのフラグを書き出す
	data32 = target->layer_data.vector_layer->flags;
	...
	// データサイズ記憶
	vector_data_size = (uint32)data_stream->data_point;
	// ラインの数書き出し
	...
	// 圧縮後のデータサイズを書き込む
	data_size = (uint32)(write_stream->data_size - compress_stream.avail_out
...
{
	FLOAT_T x, y;				// 追加した制御点の座標
	FLOAT_T pressure;			// 追加した制御点の筆圧
	FLOAT_T size;				// 追加した制御点のサイズ
...
static void AddControlPointUndo(DRAW_WINDOW* window, void* p)
{
	...
	// 制御点を削除するラインを探す
	line = (VECTOR_LINE*)layer->layer_data.vector_layer->base;
...
)
{
	...
	uint8* buff;
	// ラインのID計算用
	VECTOR_LINE* comp_line = (VECTOR_LINE*)layer->layer_data.vector_layer->base;
	// for文用のカウンタ
	unsigned int i;
...
typedef struct _ADD_TOP_LINE_CONTROL_POINT_HISTORY
{
	...
	FLOAT_T size;				// 追加した制御点のサイズ
	uint8 color[4];				// 追加した制御点の色
	FLOAT_T line_x, line_y;		// ラインの開始座標
	FLOAT_T line_pressure;		// ラインの開始筆圧
	FLOAT_T line_size;			// ラインの開始サイズ
...
static void AddTopLineControlPointUndo(DRAW_WINDOW* window, void* p)
{
	...
	// ラインのタイプ
	uint8 line_type;
	// 現在の使用ツールを判定するためにアプリケーション全体データへアクセス
	...
	{
		line = (VECTOR_LINE*)layer->layer_data.vector_layer->top_data;
		line_type = line->base_data.vector_type;
		layer->layer_data.vector_layer->top_data =
			((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->prev;
		DeleteVectorLine(&line);
		fix_flag = FALSE;
	}
	else
	...
	if(layer == window->active_layer)
	{
		if(line_type == VECTOR_TYPE_STRAIGHT
			&& app->tool_box.active_vector_brush[app->input]->brush_type == TYPE_POLY_LINE_BRUSH)
		{
			POLY_LINE_BRUSH* brush = (POLY_LINE_BRUSH*)app->tool_box.active_vector_brush[app->input]->brush_data;
			if((brush->flags & POLY_LINE_START) != 0)
			{
				...
				else if(line != NULL)
				{
					...
					{
						layer->layer_data.vector_layer->flags = VECTOR_LAYER_RASTERIZE_ACTIVE;
						layer->layer_data.vector_layer->active_data = (void*)line;
						brush->flags |= POLY_LINE_START;
						DeleteVectorLineLayer(&line->base_data.layer);
					}
				}
			...
			&& window->app->tool_box.active_vector_brush[app->input]->brush_type == TYPE_BEZIER_LINE_BRUSH)
		{
			BEZIER_LINE_BRUSH* brush = (BEZIER_LINE_BRUSH*)app->tool_box.active_vector_brush[app->input]->brush_data;
	...
	// ラインをラスタライズ
	if(layer->layer_data.vector_layer->flags != VECTOR_LAYER_RASTERIZE_ACTIVE)
	{
		layer->layer_data.vector_layer->flags = VECTOR_LAYER_RASTERIZE_TOP;
		if(fix_flag != FALSE)
		{
			layer->layer_data.vector_layer->flags |= VECTOR_LAYER_FIX_LINE;
		}
		RasterizeVectorLayer(window, layer, layer->layer_data.vector_layer);
	}
	else
	{
		...
		if(float_top != FALSE)
		{
			before_top = (VECTOR_LINE*)layer->layer_data.vector_layer->top_data;
			((VECTOR_BASE_DATA*)before_top->base_data.prev)->next = NULL;
			DeleteVectorLineLayer(&((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->layer);
		}
		if(fix_flag != FALSE)
		{
			layer->layer_data.vector_layer->flags |= VECTOR_LAYER_FIX_LINE;
	...
	layer->layer_data.vector_layer->active_data = NULL;
	(void)memset(window->work_layer->pixels, 0, window->pixel_buf_size);
...
static void AddTopLineControlPointRedo(DRAW_WINDOW* window, void* p)
{
	...
	{	// ライン追加
		line = CreateVectorLine((VECTOR_LINE*)layer->layer_data.vector_layer->top_data,
			(VECTOR_LINE*)((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->next);
		// 制御点バッファ作成
		...
		line->num_points = 1;
		// 一番上のライン変更
		layer->layer_data.vector_layer->top_data = (void*)line;
	...
	if(layer == window->active_layer)
	{
			...
			&& app->tool_box.active_vector_brush[app->input]->brush_type == TYPE_POLY_LINE_BRUSH)
		{
			...
			&& app->tool_box.active_vector_brush[app->input]->brush_type == TYPE_BEZIER_LINE_BRUSH)
		{
			BEZIER_LINE_BRUSH* brush = (BEZIER_LINE_BRUSH*)app->tool_box.active_vector_brush[app->input]->brush_data;
			brush->flags |= BEZIER_LINE_START;
			fix_flag = FALSE;
...
static void AddTopSquareUndo(DRAW_WINDOW* window, void* p)
{
	...
	// 一番上の四角形を削除
	square = (VECTOR_SQUARE*)layer->layer_data.vector_layer->top_data;
	layer->layer_data.vector_layer->top_data = ((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->prev;
	...
	else
	{
		...
		before_top = (VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data;
		layer->layer_data.vector_layer->top_data = ((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->prev;
...
static void AddTopSquareRedo(DRAW_WINDOW* window, void* p)
{
	...
	// 四角形追加
	square = (VECTOR_SQUARE*)CreateVectorShape((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data,
			(VECTOR_BASE_DATA*)((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->next, VECTOR_TYPE_SQUARE);
	before_base = square->base_data;
	*square = history.square;
	square->base_data = before_base;
	// 一番上のベクトル変更
	layer->layer_data.vector_layer->top_data = (void*)square;

	if(layer == window->active_layer)
	{
		if(app->tool_box.active_vector_brush[app->input]->brush_type == TYPE_VECTOR_SHAPE_BRUSH)
		{
...
static void AddTopEclipseRedo(DRAW_WINDOW* window, void* p)
{
	...
	// 楕円追加
	eclipse = (VECTOR_ECLIPSE*)CreateVectorShape((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data,
		(VECTOR_BASE_DATA*)((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->next, history.eclipse.base_data.vector_type);
	before_base = eclipse->base_data;
	*eclipse = history.eclipse;
	eclipse->base_data = before_base;
	// 一番上のベクトル変更
	layer->layer_data.vector_layer->top_data = (void*)eclipse;
...
static void AddTopScriptUndo(DRAW_WINDOW* window, void* p)
{
	...
	// 一番上のスクリプトを削除
	script = (VECTOR_SCRIPT*)layer->layer_data.vector_layer->top_data;
	layer->layer_data.vector_layer->top_data = ((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->prev;
...
static void AddTopScriptRedo(DRAW_WINDOW* window, void* p)
{
	...
	// スクリプト追加
	script = (VECTOR_SCRIPT*)CreateVectorScript((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data,
			(VECTOR_BASE_DATA*)((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->top_data)->next, history.script_data);
	// 一番上のベクトル変更
	layer->layer_data.vector_layer->top_data = (void*)script;
...
static void DeleteVectorShapeUndo(DRAW_WINDOW* window, void* p)
{
	...
	shape = (VECTOR_BASE_DATA*)((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->base);
	...
	layer->layer_data.vector_layer->active_data = (void*)shape;
	if(shape->next == NULL)
	{
		layer->layer_data.vector_layer->top_data = (void*)shape;
	}
	layer->layer_data.vector_layer->flags |= VECTOR_LAYER_FIX_LINE | VECTOR_LAYER_RASTERIZE_ACTIVE;

	RasterizeVectorLayer(window, layer, layer->layer_data.vector_layer);

	layer->layer_data.vector_layer->active_data = (void*)NULL;
}
...
static void DeleteVectorShapeRedo(DRAW_WINDOW* window, void* p)
{
	...
	if(shape->next == NULL)
	{
		layer->layer_data.vector_layer->top_data = shape->prev;
	}
...
)
{
	...
	VECTOR_LAYER *layer_data = window->active_layer->layer_data.vector_layer;
...
static void DeleteVectorScriptUndo(DRAW_WINDOW* window, void* p)
{
	...
	script = (VECTOR_BASE_DATA*)((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->base);
	for(i=0; i<history_data->vector_index; i++)
	{
		script = script->next;
	}

	{
		VECTOR_SCRIPT *add = (VECTOR_SCRIPT*)CreateVectorScript(script, (VECTOR_BASE_DATA*)script->next,
			script_data);
		layer->layer_data.vector_layer->active_data = (void*)add;
		script = &add->base_data;
	}

	layer->layer_data.vector_layer->active_data = (void*)script;
	if(script->next == NULL)
	{
		layer->layer_data.vector_layer->top_data = (void*)script;
	}
...
static void DeleteVectorScriptRedo(DRAW_WINDOW* window, void* p)
{
	...
	if(script->next == NULL)
	{
		layer->layer_data.vector_layer->top_data = script->prev;
	}
...
void DivideLinesUndo(DRAW_WINDOW* window, void* p)
{
	...
	while(target != NULL)
	{
		...
		{
			layer = target->layer_data.vector_layer;
			break;
		}
		target = target->next;
	}
...
static void DeleteLinesUndo(DRAW_WINDOW* window, void* p)
{
	...
	while(line != NULL)
	{
		if(i == data.indexes[exec_lines])
		{
			...
			if((void*)line == layer->layer_data.vector_layer->top_data)
			{
				layer->layer_data.vector_layer->top_data = (void*)line;
			}
...
static void DeleteLinesRedo(DRAW_WINDOW* window, void* p)
{
	...
	line = (VECTOR_LINE*)((VECTOR_BASE_DATA*)layer->layer_data.vector_layer->base)->next;
	i = 0;
	while(line != NULL)
	{
		if(i == data.indexes[exec_lines])
