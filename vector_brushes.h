...
typedef struct _FREE_HAND_BRUSH
{
	int source;
	int base_scale;
	FLOAT_T r, blur, outline_hardness;
	FLOAT_T before_x, before_y;
	FLOAT_T draw_before_x, draw_before_y;
	FLOAT_T before_arg;
	FLOAT_T min_degree, min_arg;
...
typedef struct _VECTOR_SHAPE_BRUSH
{
	...
	eGRAPHICS_LINE_JOIN join_type;	// 線の繋ぎ目の形
	FLOAT_T shape_points[9][2];		// 拡大縮小・回転・移動用の座標記憶バッファ
	VECTOR_DATA before_shape;		// 形状変更前のデータ
	void *detail_widget;			// 各モードの設定ウィジェット
	void *detail_frame;		 	// 各モードの設定ウィジェットのコンテナ
